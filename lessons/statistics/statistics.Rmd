...
library(dplyr)
library(ggplot2)
knitr::opts_chunk$set(comment = '', fig.height = 6, fig.width = 8, fig.align = 'center')
```
...
```{r kmeans1}
cats <- cats %>% 
  mutate(scale_weight = scale(weight) %>% as.numeric,
         scale_wander = scale(wander_dist) %>% as.numeric,
         scale_age = scale(age) %>% as.numeric)
```
